...
import { formatPercent, formatR, formatDateTime } from '@/lib/services';
    ...
    for (const t of closedTrades) {
      ...
      if (!t.followed_plan) {
        violations.push({
          ...
          date: t.entry_time,
          type: 'Plan Deviation',
          detail: 'Trade did not follow the plan',
        });
      }
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-[#8B8BA7] text-sm mt-1">Monitor your risk compliance and discipline</p>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="bg-[#111118] border-[#1E1E2E]">
                <CardContent className="p-4">
                  <p className="text-xs text-[#8B8BA7] uppercase tracking-wider">Compliance Rate</p>
                  ...
                  <p className="text-xs text-[#8B8BA7] mt-1">{compliantTrades.length}/{closedTrades.length} trades</p>
                </CardContent>
              </Card>
              <Card className="bg-[#111118] border-[#1E1E2E]">
                <CardContent className="p-4">
                  <p className="text-xs text-[#8B8BA7] uppercase tracking-wider">Avg Planned Risk</p>
                  <p className="text-2xl font-bold font-mono text-white mt-1">{avgPlannedRisk.toFixed(2)}%</p>
                </CardContent>
              </Card>
              <Card className="bg-[#111118] border-[#1E1E2E]">
                <CardContent className="p-4">
                  <p className="text-xs text-[#8B8BA7] uppercase tracking-wider">Avg Actual Risk</p>
              ...
              <Card className="bg-[#111118] border-[#1E1E2E]">
                <CardContent className="p-4">
                  <p className="text-xs text-[#8B8BA7] uppercase tracking-wider">Violations</p>
                ...
                {activeRule ? (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-[#0A0A0F] rounded-lg p-3">
                      <p className="text-xs text-[#8B8BA7]">Max Risk/Trade</p>
                      <p className="font-mono font-bold text-white">{activeRule.max_risk_per_trade}%</p>
                    </div>
                    {activeRule.max_daily_loss && (
                      <div className="bg-[#0A0A0F] rounded-lg p-3">
                        <p className="text-xs text-[#8B8BA7]">Max Daily Loss</p>
                        <p className="font-mono font-bold text-white">{activeRule.max_daily_loss}%</p>
                      </div>
                    )}
                    {activeRule.max_weekly_loss && (
                      <div className="bg-[#0A0A0F] rounded-lg p-3">
                        <p className="text-xs text-[#8B8BA7]">Max Weekly Loss</p>
                        <p className="font-mono font-bold text-white">{activeRule.max_weekly_loss}%</p>
                      </div>
                    )}
                    <div className="bg-[#0A0A0F] rounded-lg p-3">
                      <p className="text-xs text-[#8B8BA7]">Max Consecutive Losses</p>
                      <p className="font-mono font-bold text-white">{activeRule.max_consecutive_losses}</p>
                    </div>
                  </div>
                ) : (
                  <p className="text-[#8B8BA7] text-sm">No active risk rule. Go to Settings to create one.</p>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent>
                {violations.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-[#8B8BA7] text-sm">No violations found. Great discipline!</p>
                  </div>
                ) : (
                    ...
                    {violations.slice(0, 20).map((v, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-[#0A0A0F] rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm text-white font-mono">{v.instrument}</p>
                            <p className="text-xs text-[#8B8BA7]">{formatDateTime(v.date)}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <Badge variant="outline" className="text-[#FF4D6D] border-[#FF4D6D]/30 text-[10px]">{v.type}</Badge>
                          <p className="text-xs text-[#8B8BA7] mt-1">{v.detail}</p>
                        </div>
                      </div>
                    ))}
                  </div>
