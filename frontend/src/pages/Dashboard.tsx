...
function KpiCard({ title, value, subtitle, icon: Icon, color }: {
  ...
  return (
    <Card className={`bg-[#111118] border-[#1E1E2E] border-l-4 ${borderColor}`}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-[#8B8BA7] uppercase tracking-wider">{title}</span>
          <Icon className={`w-4 h-4 ${iconColor}`} />
        </div>
        <p className="text-2xl font-bold font-mono text-white">{value}</p>
        {subtitle && <p className="text-xs text-[#8B8BA7] mt-1">{subtitle}</p>}
      </CardContent>
    </Card>
  );
}
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-[#8B8BA7] text-sm mt-1">Your trading performance overview</p>
        </div>
        ...
        {loading ? (
          ...
          <Card className="bg-[#111118] border-[#1E1E2E]">
            <CardContent className="flex flex-col items-center justify-center py-16">
              <Activity className="w-12 h-12 text-[#8B8BA7] mb-4" />
              <h3 className="text-lg font-semibold text-white mb-2">No trades yet</h3>
              <p className="text-[#8B8BA7] text-sm mb-4">Start logging your trades to see analytics</p>
          ...
          <>
            ...
            {chartData.length > 0 && (
              <Card className="bg-[#111118] border-[#1E1E2E]">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-[#8B8BA7] uppercase tracking-wider">
            ...
            <Card className="bg-[#111118] border-[#1E1E2E]">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm text-[#8B8BA7] uppercase tracking-wider">Recent Trades</CardTitle>
                <Link to="/trades" className="text-xs text-indigo-400 hover:text-indigo-300">View All →</Link>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {recentTrades.map((trade) => (
                    <div key={trade.id} className="flex items-center justify-between p-3 rounded-lg bg-[#0A0A0F] hover:bg-[#0F0F18] transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-white font-mono">{trade.instrument}</p>
                          <p className="text-xs text-[#8B8BA7]">{formatDateTime(trade.entry_time)}</p>
                        </div>
                      </div>
